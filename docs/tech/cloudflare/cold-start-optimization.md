...
L'architecture proposée repose sur une segmentation stricte du trafic. Il est impératif de distinguer les requêtes qui *peuvent* être servies depuis le cache de celles qui *doivent* déclencher une exécution serveur. Dans un contexte CMS, cette distinction se fait principalement via l'authentification.  
...
La stratégie viable consiste à **conditionner l'inclusion des plugins** lors de la phase de build ou via des variables d'environnement, et à utiliser des imports dynamiques pour les fonctionnalités exécutées *au moment de la requête* (runtime) plutôt qu'au démarrage (boot time).
