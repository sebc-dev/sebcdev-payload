...
module.exports = {  
  forbidden:     
    {  
      ...
      severity: 'error',  
      comment: 'Le Domaine doit rester agnostique du framework (pas de React, Angular, NestJS dans le domaine).',  
...
```
Cette configuration illustre une compréhension nuancée : on n'interdit pas *tous* les node_modules (car le domaine peut avoir besoin de zod ou decimal.js), mais on cible spécifiquement les frameworks qui coupleraient le métier à une technologie d'implémentation.
...
3. Le système passe au vert tant que *nouvelles* violations ne sont pas ajoutées. Les anciennes sont ignorées silencieusement.  
...
```
L'utilisation de --collapse est une technique puissante. Elle regroupe tous les fichiers d'un dossier en une seule boîte. Cela permet de visualiser les dépendances entre *modules* (Domain -> Infra) plutôt qu'entre fichiers, rendant le diagramme immédiatement compréhensible pour les parties prenantes non techniques.
